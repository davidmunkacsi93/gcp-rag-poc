"""
Load regional_metrics seed data into Cloud SQL (GCP).

Reads connection details from environment variables prefixed with CLOUD_SQL_.
Auto-generates seed data if missing.
"""
...
def load() -> int:
    ...
    print(f"Loaded {len(rows)} rows into Cloud SQL regional.regional_metrics")
    return len(rows)
